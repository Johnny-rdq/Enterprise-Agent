...
- 游戏窗口 600x600 起，背景深色
- 完整的游戏循环、得分显示、游戏结束判断
- 严禁加载任何外部图片/音效文件（如 player.png），所有图形必须用 pygame.draw 绘制（矩形、圆形、多边形等）
- 用 pip install 安装缺失的包时，系统会自动补装
- 【硬性要求】玩家必须有至少3条命，碰撞后扣命并短暂无敌（闪烁），不能一碰就死
- 【硬性要求】命用完必须显示"游戏结束"画面：展示最终得分，提示"按R重新开始，按Q退出"
- 【硬性要求】游戏结束后不能直接 pygame.quit() 或 sys.exit()，必须等待用户按键选择
- pygame 游戏模板：生命耗尽 → 显示game over文字+得分 → 等用户按R重置状态或按Q退出 → 按Q才退出循环"""
