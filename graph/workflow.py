...
def save_code_node(state: AgentState) -> dict:
    ...
    # 尝试自动打开文件（Windows: 用默认编辑器；Mac/Linux: 尝试 open/xdg-open）
    try:
        if os.name == "nt":
...
workflow.add_node("save_code_node", save_code_node)       # 保存代码到文件
...
# 审查后的条件跳转：通过→保存代码→结束，未通过→打回重写
workflow.add_conditional_edges(
    "review_node",
    review_decision,
    {
        "save_code": "save_code_node",   # 通过 → 保存代码文件
